...
try {
  ...
  if (args.length !== 4) {
    ...
    var url = args[1];
    var div = args[2];
    div = decodeURIComponent(div.replace(/\+/g, ' '));
    var json = JSON.parse(div);
    var dir = args[3];
    if (!Array.isArray(Object.keys(json))) {
      console.log(JSON.stringify("no array"));
      phantom.exit();
    }
        ...
        for (var key in (json)) {
          ...
          waitForGlobal(key, page, (new Date()).getTime() + 5000, function(status) {
            if (status) {
              ...
              console.log(JSON.stringify("CLICKTRAIT_OK"));
              if (!test) {
